export function validarEvento(body) {
  if (!body || typeof body.id !== 'string' || !/^[0-9a-f-]{36}$/i.test(body.id)) throw new Error('Evento inválido.')
  ...
  return { id:body.id, title:body.title.trim(), description:body.description?.trim() || null, location:body.location?.trim() || null, all_day:body.all_day, starts_at:body.all_day ? body.starts_at : ini.toISOString(), ends_at:body.all_day ? body.ends_at : fin.toISOString(), time_zone:timeZone }
}
